...
**Session**:
A record of one agent interaction, reported by the agent itself over MCP as a structured summary (task, conclusion, key snippets), including model, tokens, skills used, and cost. Full original context is stored only when the user explicitly asks to remember.
